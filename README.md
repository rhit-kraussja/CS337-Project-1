...
### Train Model
We would need to train the spacy model beforehand in order gain more accurate results with the data (such as getting nominees and award winners). To train, run `python -m spacy train config.cfg --output ./output --paths.train ./train.spacy --paths.dev ./dev.spacy` once all the installation has been completed.
